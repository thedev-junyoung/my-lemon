...
from app.core.dependencies import get_current_user, get_http_exception_handler
from typing import List

router = APIRouter()

@router.get("/", response_model=SuccessResponse, dependencies=[Depends(get_http_exception_handler)])
def read_users(db: Session = Depends(get_db), token: dict = Depends(get_current_user)):
...
def read_user(user_id: int, db: Session = Depends(get_db), token: dict = Depends(get_current_user)):
...
def update_user_endpoint(user_id: int, user: UserUpdate, db: Session = Depends(get_db), token: dict = Depends(get_current_user)):
    if user_id < 1:
...
def delete_user_endpoint(user_id: int, db: Session = Depends(get_db), token: dict = Depends(get_current_user)):
    if user_id < 1:
        raise HTTPException(status_code=400, detail="유효하지 않은 사용자 ID입니다.")
